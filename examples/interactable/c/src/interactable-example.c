...
export void websg_load() {
  ...
  material_button = websg_world_find_node_by_name(material_button_name, strlen(material_button_name));
  const char *left_cube_name = "LeftCube";
  left_cube = websg_world_find_node_by_name(left_cube_name, strlen(left_cube_name));
  const char *right_cube_name = "RightCube";
  right_cube = websg_world_find_node_by_name(right_cube_name, strlen(right_cube_name));
  const char *room1_switch_name = "Room1Switch";
  room1_switch = websg_world_find_node_by_name(room1_switch_name, strlen(room1_switch_name));
  ...
  InteractableProps props = { .type = InteractableType_Interactable };

  websg_node_add_interactable(material_button, &props);
  websg_node_add_interactable(room1_switch, &props);
  websg_node_add_interactable(room2_switch, &props);
}

export void websg_update(float_t dt) {
  if (websg_node_get_interactable_pressed(material_button)) {
