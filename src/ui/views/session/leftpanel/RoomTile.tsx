...
export function RoomTile({ name, roomColor, avatarUrl, isActive = false, onClick }: IRoomTile) {
  return (
    <button onClick={onClick} className={`RoomTile${isActive ? "--active" : ""} flex items-start`}>
      <Thumbnail className="shrink-0" name={name} bgColor={roomColor} imageSrc={avatarUrl} />
