  ...
  return (
    <div className="RoomPreview grow flex flex-column">
      <header className="flex items-center justify-end">
        <Button variant="primary" onClick={() => alert("create room")}>
          Create room
        </Button>
        <Avatar
          isCircle
      ...
      <Scroll className="grow">
        <div className="RoomPreview__content">
          <Thumbnail size="large" name={roomName} bgColor={vm.getRoomColor()} imageSrc={vm.getRoomAvatarHttpUrl()} />
          <Text variant="h2" weight="semi-bold">
            {roomName}
          </Text>
          <Button variant="primary" onClick={() => vm.setRoomFlow("load")}>
            Join room
          </Button>
        </div>
      </Scroll>
