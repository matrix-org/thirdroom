...
static JSValue js_mesh_primitive_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
  ...
  JSValue val = JS_NewObjectProtoClass(ctx, proto, js_mesh_primitive_class_id);
  JS_FreeValue(ctx, proto);
...
static JSValue js_mesh_primitive_get_name(JSContext *ctx, JSValueConst this_val) {
  ...
  if (!mesh_primitive) {
    ...
    JSValue val;
    val = JS_NewString(ctx, mesh_primitive->name);
    return val;
  }
}


static JSValue js_mesh_primitive_set_name(JSContext *ctx, JSValueConst this_val, JSValue val) {
  MeshPrimitive *mesh_primitive = JS_GetOpaque2(ctx, this_val, js_mesh_primitive_class_id);

  if (!mesh_primitive) {
    return JS_EXCEPTION;
  } else {
    mesh_primitive->name = JS_ToCString(ctx, val);
    return JS_UNDEFINED;
  }
}
...
static const JSCFunctionListEntry js_mesh_primitive_proto_funcs[] = {
  JS_CGETSET_DEF("name", js_mesh_primitive_get_name, js_mesh_primitive_set_name),
