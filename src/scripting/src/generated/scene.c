...
static JSValue js_scene_get_bloom_threshold(JSContext *ctx, JSValueConst this_val) {
  Scene *scene = JS_GetOpaque2(ctx, this_val, js_scene_class_id);
  ...
  if (!scene) {
    ...
    JSValue val;
    val = JS_NewFloat64(ctx, (double)scene->bloom_threshold);
    return val;
  }
}


static JSValue js_scene_set_bloom_threshold(JSContext *ctx, JSValueConst this_val, JSValue val) {
  Scene *scene = JS_GetOpaque2(ctx, this_val, js_scene_class_id);

  if (!scene) {
    return JS_EXCEPTION;
  } else {
    if (JS_ToFloat32(ctx, &scene->bloom_threshold, val)) return JS_EXCEPTION;
    return JS_UNDEFINED;
  }
}


static JSValue js_scene_get_bloom_radius(JSContext *ctx, JSValueConst this_val) {
  Scene *scene = JS_GetOpaque2(ctx, this_val, js_scene_class_id);
  ...
  if (!scene) {
    ...
    JSValue val;
    val = JS_NewFloat64(ctx, (double)scene->bloom_radius);
    return val;
  }
}


static JSValue js_scene_set_bloom_radius(JSContext *ctx, JSValueConst this_val, JSValue val) {
  Scene *scene = JS_GetOpaque2(ctx, this_val, js_scene_class_id);

  if (!scene) {
    return JS_EXCEPTION;
  } else {
    if (JS_ToFloat32(ctx, &scene->bloom_radius, val)) return JS_EXCEPTION;
    return JS_UNDEFINED;
  }
}
...
static const JSCFunctionListEntry js_scene_proto_funcs[] = {
  ...
  JS_CGETSET_DEF("bloomThreshold", js_scene_get_bloom_threshold, js_scene_set_bloom_threshold),
  JS_CGETSET_DEF("bloomRadius", js_scene_get_bloom_radius, js_scene_set_bloom_radius),
