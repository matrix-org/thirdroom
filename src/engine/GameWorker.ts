...
import { InitializeGameWorkerMessage, WorkerMessages, WorkerMessageType, GameWorkerInitializedMessage, GameWorkerErrorMessage } from './WorkerMessage';
...
const onMessage = (state: World) => ({ data }: any) => {
...
const generateInputGetters = (inputStates: InputState[], inputTripleBuffer: TripleBufferState): InputObjectType => InputArray.reduce((a, v, i) =>
...
async function onInit({
    ...
    createRemoteResourceManager(resourceManagerBuffer, renderPort);
...
const cubeMoveSystem = ({ input, physics }: GameState) => {
    ...
    if (input.KeyW) {rigidBody.applyImpulse(forward, true);}
    if (input.KeyS) {rigidBody.applyImpulse(backward, true);}
    if (input.KeyA) {rigidBody.applyImpulse(left, true);}
    if (input.KeyD) {rigidBody.applyImpulse(right, true);}
    if (input.Space) {rigidBody.applyImpulse(up, true);}
};

const cameraMoveSystem = ({ input, time: { delta } }: GameState) => {
    const eid = 0;
    const position = Transform.position[eid];
    if (input.ArrowUp) {position[2] -= delta * 25;}
    if (input.ArrowDown) {position[2] += delta * 25;}
    if (input.ArrowLeft) {position[0] -= delta * 25;}
    if (input.ArrowRight) {position[0] += delta * 25;}
};
