...
async function onInitMessage(event: MessageEvent) {
  if (typeof event.data !== "object") {
    return;
  }

  const message = event.data as WorkerMessages;
...
async function onInit({ initialGameWorkerState, renderWorkerMessagePort }: InitializeGameWorkerMessage) {
  ...
  await registerModules({ ...initialGameWorkerState, renderPort }, state, gameConfig.modules);
