...
static void js_websg_vector3_finalizer(JSRuntime *rt, JSValue val) {
  WebSGVector3Data *vec3_data = JS_GetOpaque(val, websg_vector3_class_id);
...
void js_websg_define_vector3(JSContext *ctx) {
  JS_NewClassID(&websg_vector3_class_id);
  JS_NewClass(JS_GetRuntime(ctx), websg_vector3_class_id, &websg_vector3_class);
  JSValue world_proto = JS_NewObject(ctx);
  JS_SetPropertyFunctionList(ctx, world_proto, js_websg_vector3_proto_funcs, countof(js_websg_vector3_proto_funcs));
  JS_SetClassProto(ctx, websg_vector3_class_id, world_proto);
}
...
JSValue js_websg_new_vector3_get_set(
  ...
  JSValue vector3 = JS_NewObjectClass(ctx, websg_vector3_class_id);
