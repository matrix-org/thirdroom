...
export interface IGameNetworkScope {
  hosting: boolean;
...
export const NetworkScope: () => IGameNetworkScope = () => ({
  hosting: false,
  incoming: [],
...
export async function NetworkModule(ctx: GameState) {
  const disposables = [
    ...
    if (haveNewPeers) {
      ...
      while (network.newPeers.length) {
        ...
        if (theirPeerId) {
          ...
          if (network.hosting) broadcastReliable(state, createPeerIdIndexMessage(state, theirPeerId));
          ...
          sendReliable(state, theirPeerId, createPlayerNetworkIdMessage(state, network.peerId));
        }
      }
    } else {
...
const processNetworkMessage = (state: GameState, msg: ArrayBuffer) => {
  ...
  const input: NetPipeData = [state, cursorView];
  const { messageHandlers } = getScope(state, NetworkScope);
